...
import { nftContractAddress, nftPrice } from '../config.js'
...
import NFT from '../utils/FantasticBeastsNFT.json'
...
const mint = () => {
  ...
  const getMintedNFT = async () => {
    try {
      ...
      if (ethereum) {
        ...
        let nftTx = await nftContract.getBeastsOfOwner()
        console.log('Mining....', nftTx)

        try {
          const items = await Promise.all(nftTx.map(async (i) => {
            console.log('i', i);
  ...
  if(mintedNFT) {
    const listItems = mintedNFT.map((item, index) =>
      <img
        key={index}
        src={item.image}
        alt={'item-' + index}
  ...
  const getMintedNFT2 = async (tokenId) => {
    try {
      const { ethereum } = window
  ...
  return (
    <div className='flex flex-col items-center pt-32 bg-[#0c343d] text-[#d3d3d3] min-h-screen'>
      ...
      <h2 className='text-3xl font-bold mb-20 mt-12'>
        Mint your Fantastic Beasts NFT!
      </h2>
      {currentAccount === '' ? (
        <button
